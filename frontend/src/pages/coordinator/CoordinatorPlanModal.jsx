...
import { X, Upload, Trash2, Download, FileText, CheckCircle, Search, Wrench } from 'lucide-react'
...
export default function CoordinatorPlanModal({ request, onClose, onSaved }) {
  ...
  const handleSubmit = async () => {
    if (!window.confirm('Planı bitirmək və təklifi göndərmək istəyirsiniz?')) return
    setSubmitting(true)
    try {
      await doSave()
      await coordinatorApi.submitPlan(request.requestId)
      toast.success('Təklif göndərildi — sorğu OFFER_SENT statusuna keçdi')
      onSaved()
      onClose()
    } catch (err) {
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-3xl relative flex flex-col max-h-[90vh]">
          ...
          <div className="overflow-y-auto flex-1 p-6 space-y-4 scrollbar-thin">
            ...
            <div>
              <label className={labelCls}>Operator adı</label>
              <input type="text" value={form.operatorName} onChange={(e) => set('operatorName', e.target.value)}
                placeholder="Operator adı soyadı" className={inputCls} disabled={isReadonly} />
            </div>
