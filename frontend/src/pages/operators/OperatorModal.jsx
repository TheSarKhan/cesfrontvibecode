...
export default function OperatorModal({ editing, onClose, onSaved }) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    if (!form.firstName.trim() || !form.lastName.trim()) return toast.error('Ad və Soyad tələb olunur')
    setSaving(true)
    try {
      if (editing) {
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <Field label="Ad *" name="firstName" placeholder="Əli" />
            <Field label="Soyad *" name="lastName" placeholder="Məmmədov" />
          </div>
