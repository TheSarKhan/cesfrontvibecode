...
export default function ServicePage() {
  ...
  const handleDelete = async (rec) => {
    if (!(await confirm({ title: 'Servis qeydini sil', message: `"${rec.serviceType} — ${rec.equipmentName}" servis qeydi silinsin?` }))) return
    try {
            ...
            <tbody>
              {loading ? (
                ...
                <EmptyState
                  ...
                  description={records.length === 0 ? 'İlk servis qeydini əlavə edin' : 'Axtarış şərtlərini dəyişin'}
                  action={canCreate ? () => setModal({ open: true, editing: null }) : undefined}
                  actionLabel={canCreate ? 'Yeni Servis' : undefined}
                />
              ) : (
                filtered.map(rec => (
