...
export default function DebitCreditPage() {
  ...
  const [stats, setStats] = useState({ totalDebt: 0, totalPaid: 0, overdueCount: 0, pendingCount: 0 })
  ...
  const loadReceivables = async () => {
    ...
    } catch { toast.error('Debitor gətirilərkən xəta baş verdi') }
    finally { setLoading(false) }
  }

  useEffect(() => { loadReceivables() }, [statusFilter]) // eslint-disable-line react-hooks/exhaustive-deps
  ...
  return (
    <div style={{ color: 'var(--ces-ink)' }}>
      <PageHeader
        ...
        title="Debitor"
        subtitle="Müştəri borclarının izlənməsi və ödəniş təqibi"
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-5">
        <KpiBlock label="Ümumi Gözlənilən" value={fmtMoney(stats.totalDebt)} color="var(--ces-graphite-900)" />
        <KpiBlock label="İndiyə qədər ödənilib" value={fmtMoney(stats.totalPaid)} color="var(--ces-ok)" />
        <KpiBlock label="Gecikmiş Ödənişlər" value={`${stats.overdueCount} qeyd`} color="var(--ces-danger)" tone="danger" icon={AlertCircle} />
        <KpiBlock label="Gözləyən/Qismən" value={`${stats.pendingCount} qeyd`} color="var(--ces-info)" tone="info" icon={Clock} />
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 justify-between mb-4">
          ...
          {['ALL', 'PENDING', 'PARTIAL', 'OVERDUE', 'COMPLETED'].map(status => {
            ...
            return (
              <button key={status} onClick={() => setStatusFilter(status)}
                ...
                {status === 'ALL' ? 'Hamısı' : STATUS_LABEL[status]?.split(' ')[0] || status}
              </button>
            )
          })}
        </div>
        <div className="flex items-center gap-2 sm:w-72"
          ...
          <input
            placeholder="Layihə, müştəri axtar..."
            value={searchQuery}
                  ...
                  return (
                    <React.Fragment key={item.id}>
                      <tr onClick={() => handleExpand(item.id)} className="cursor-pointer"
                        ...
                        <td>
                          <p className="font-medium" style={{ color: 'var(--ces-ink)' }}>{item.customerName || '—'}</p>
                          <p className="text-[11px]" style={{ color: 'var(--ces-mute2)' }}>{item.customerVoen || '—'}</p>
                        </td>
                        <td><span className="font-bold num" style={{ color: 'var(--ces-warn)' }}>{fmtMoney(item.totalAmount)}</span></td>
                        <td>
                                ...
                                <div className="md:col-span-2 space-y-3">
                                  ...
                                  <div className="overflow-hidden"
                                    ...
                                    {!item.invoices?.length ? (
                                      <div className="p-4 text-center text-[12px]" style={{ color: 'var(--ces-mute2)' }}>Hələ qaimə kəsilməyib</div>
                                    ) : (
                                      <div className="overflow-x-auto">
                                        ...
                                        {item.payments.map((p, i) => (
                                          <div key={p.id} className="flex items-center justify-between p-3 text-[12px]"
                                            ...
                                            {item.status !== 'COMPLETED' && (
                                              <button onClick={() => handleDeletePayment(item.id, p.id)} className="ces-row-act danger">Sil</button>
                                            )}
                                          </div>
                                        ))}
                                      </div>
                                ...
                                <div className="space-y-3">
                                  ...
                                  {item.status === 'COMPLETED' ? (
                                    ...
                                    <form onSubmit={(e) => handleAddPayment(e, item.id)}
                                      ...
                                      style={{ background: 'var(--ces-surface)', border: '1px solid var(--ces-line)', borderRadius: '12px' }}>
                                      <Field label="Qaimə Seçin (könüllü)">
                                      ...
                                      <button type="submit" disabled={payLoading} className="ces-btn ces-btn-primary w-full">
                                  ...
                                  {item.status !== 'COMPLETED' && rem <= 0 && (
                                    <button onClick={() => handleComplete(item.id)}
                                      ...
                                      <Lock size={14} /> Debitoru Yekunlaşdır
                                    </button>
                                  )}
                                </div>
                              </div>
